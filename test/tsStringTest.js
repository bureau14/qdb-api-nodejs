...
describe('Timeseries - String', function () {
    ...
    describe('column info', function () {
        ...
        before('init', function () {
            col_info = qdb.StringColumnInfo('col');
        });
        ...
        it('should have type property', function () {
            ...
            test.must(col_info.type).be.equal(qdb.TS_COLUMN_STRING);
        });
    }); // column info
    ...
    describe('ts', function () {
        ...
        var ts = null
        before('init', function () {
            columnInfo = qdb.StringColumnInfo('col')
            ts = insecureCluster.ts('ts')
        });
        
        it('should create string column', function (done) {
            ts.remove(function (err) {
                ts.create([columnInfo], function (err, cols) {
                    ...
                    test.must(cols[0].type).be.equal(qdb.TS_COLUMN_STRING);
        ...
        it('should not create existing string column', function (done) {
        ...
        it('should remove timeseries with string column', function (done) {
            ts.remove(function (err) {
                test.must(err).be.equal(null);
    ...
    describe('column', function () {
        ...
        it('should insert string column', function (done) {
        ...
        it('should show string column', function (done) {
            ts.columns(function (err, columns) {
                test.must(err).be.equal(null);
        ...
        it('should remove ts with string column', function (done) {
            ts.remove(function (err) {
                test.must(err).be.equal(null);
    ...
    describe('insert', function () {
        ...
        before('init', function (done) {
            var columnInfo = qdb.StringColumnInfo('col')
            ts = insecureCluster.ts('ts')
            ...
            ts.remove(function (err) {
                ts.create([columnInfo], function (err, cols) {
                    ...
                    test.should(cols[0].type).eql(qdb.TS_COLUMN_STRING);
                    column = cols[0];
        ...
        it('should create point', function () {
            ...
            var point = qdb.StringPoint(timestamp, value);
        ...
        it('should hold string value', function () {
            var date = new Date(2049, 10, 5);
            var timestamp = qdb.Timestamp.fromDate(date);
            var value = Buffer.from('content', 'utf8');
            var point = qdb.StringPoint(timestamp, value);
        ...
        it('should insert multiple string points', function (done) {
            var points = [
    ...
    describe('ranges', function () {
        ...
        it('should insert string points', function (done) {
            insertedPoints = [
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2030, 10, 5, 6)), Buffer.from('a', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2030, 10, 5, 7)), Buffer.from('b', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2030, 10, 5, 8)), Buffer.from('c', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2049, 10, 5, 1)), Buffer.from('d', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2049, 10, 5, 2)), Buffer.from('e', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2049, 10, 5, 3)), Buffer.from('f', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2049, 10, 5, 4)), Buffer.from('g', 'utf8')),
                qdb.StringPoint(qdb.Timestamp.fromDate(new Date(2049, 10, 5, 5)), Buffer.from('h', 'utf8')),
            ];
        ...
        it('should retrieve string points in range', function (done) {
            ...
            column.ranges([range], function (err, points) {
                ...
                stringsCheck(exp, points);
                done();
            });
        });
        

        it('should retrieve string points in all ranges', function (done) {
        ...
        it('should retrieve all string points', function (done) {
            ...
            column.ranges([range], function (err, points) {
                test.must(err).be.equal(null);
                stringsCheck(insertedPoints, points);

                done();
            });
        });

        it('should erase ranges of string points', function (done) {
        ...
        it('should retrieve all remaining string points', function (done) {
            var begin = new Date(2000, 10, 5, 2);
            var end = new Date(2049, 10, 5, 10);
            var range = qdb.TsRange(qdb.Timestamp.fromDate(begin), qdb.Timestamp.fromDate(end));
    ...
    describe('aggregations', function () {
        ...
        before('init', function (done) {
            ...
            ts.remove(function (err) {
                ts.create([qdb.StringColumnInfo('col')], function (err, cols) {
        ...
        it('should find first and last string points', function (done) {
